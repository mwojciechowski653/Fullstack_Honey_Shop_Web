...
async function fetchCustomers() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch customers: ${response.statusText} (status: ${response.status})`);
        }
...
function renderCustomers(customers) {
    ...
    customers.forEach(customer => {
        ...
        addressDiv.innerHTML = `${customer.street || ''} ${customer.street_number || ''}, ${customer.postal_code || ''}` || 'N/A';
...
document.getElementById('search-button').addEventListener('click', fetchCustomers);
