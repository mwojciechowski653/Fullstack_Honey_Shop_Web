...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    expiry.setFullYear(
      parseInt("20" + expiryDate[0]),
      parseInt(expiryDate[1]) - 1,
      1
    );
    if (expiry < today) {
      alert("Card expired");
    ...
    const deliveryOptionId =
      JSON.parse(localStorage.getItem("deliveryOptionId")) || 1;
    ...
    alert(
      `Order placed successfully. Your order number is ${response.orderId}`
    );
...
async function getCartSummary() {
  ...
  const deliveryOptionId =
    JSON.parse(localStorage.getItem("deliveryOption")) || 1;
  ...
  for (const cartItem of cartItems) {
    ...
    const item = cart.find(
      (item) => item.sizeOptionId === cartItem.size_option_id
    );
    const {
      product_id,
      name,
  ...
  document.getElementById("total").innerText = `Total: ${totalTotal.toFixed(
    2
  )}€`;
  document.getElementById(
    "total-quantity"
  ).innerText = `${cartQuantity} products`;
}
