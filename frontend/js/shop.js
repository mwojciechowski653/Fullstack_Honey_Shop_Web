...
async function initializeProducts() {
  ...
  if (allProducts.length > 0) {
    const maxPrice = Math.max(
      ...allProducts.map((product) => choosePrice(product))
    );
...
function renderProducts(products) {
  ...
  productsArea.innerHTML = products
    .map(
      (product) =>
        `
            <div class="reccomendation">
            <img class="reccomendation-image" id="item-image${
              product.size_options.id
            }" src="${product.image_url}" alt="reccomendation-image">
            <div class="reccomendation-title" id="item-title${
              product.size_options.id
            }">${product.name} - ${product.size_options.size}g</div>
            <div class="reccomendation-price" id="item-price${
              product.size_options.id
            }">${choosePrice(product)}€</div>
            <a href="product.html?id=${
              product.id
            }" class="reccomendation-button" id="redirection${
          product.id
        }">See details <img src="../assets/eye-icon.svg" alt="eye-icon"></a>
            </div>
        `
    )
    .join("");
...
function handleFiltering() {
  ...
  const minPrice =
    parseInt(document.getElementById("min-output").value, 10) || null;
  const maxPrice =
    parseInt(document.getElementById("max-output").value, 10) || null;
...
function filterProductsByCategory(products, categories) {
  return products.filter((product) =>
    categories.includes(categoryNumber(product.category))
  );
}
...
function filterProductsBySize(products, sizes) {
  return products.filter((product) =>
    sizes.includes(sizeNumber(product.size_options.size))
  );
}
...
document
  .getElementById("price-ascending")
  .addEventListener("click", (event) => {
    handleSortButtonClick(event, "priceAsc");
  });

document
  .getElementById("price-descending")
  .addEventListener("click", (event) => {
